...
// current phi/theta kept in module-level closure vars so the animate loop
// and the marker-projection loop stay perfectly in sync
let currentPhi = 0
let currentTheta = 0.2

/* ---------------------------------------------------------------------- */
/* Data: recent published scenarios + their language coordinates          */
/* ---------------------------------------------------------------------- */
...
function scenarioSortKey(s) {
  // Sorting by publication date makes more sense here than creation date:
  // a scenario can be created long before it's actually published.
...
async function loadData() {
  try {
    ...
    // Only resolve coordinates for the languages behind the most recent published
    // scenarios (a bounded pool, since the language catalogue itself is huge and
    // some languages have no coordinates on record — over-fetch a bit to compensate).
    ...
    await Promise.all(
      languageIds.map(async (id) => {
        try {
          ...
          // Language failed to load — just skip it, keep the others.
        }
      })
    )
    ...
    // Silently keep the bare globe if scenario/language data can't be loaded
    scenarios.value = []
  }
}
...
// Deterministic small rotation per scenario so polaroids don't all sit dead straight
...
/* ---------------------------------------------------------------------- */
/* Polaroid HTML overlay: projected from the globe's current phi/theta    */
/* ---------------------------------------------------------------------- */
...
// Mirrors cobe's own internal marker projection exactly (see cobe's `U`/`O`
// helpers), so the HTML polaroid overlay lines up pixel-for-pixel with the
// WebGL-rendered pin. cobe's projection isn't a plain textbook orthographic
// projection (the x/y axes and rotation order don't match a naive formula),
// so this has to replicate its math rather than approximate it.
...
function project(lat, lon, phi, theta) {
  ...
  // Screen-space x/y (cobe's `c`/`s`) and camera-facing depth, in cobe's own
  // rotation convention: yaw (phi) around Y first, then pitch (theta) around X.
