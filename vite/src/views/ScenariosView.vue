...
<template>
  <main class="ms-root">
    ...
    <div class="ms-bar">
      <div class="ms-search">
        <svg class="ms-search__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      ...
      <div class="ms-tabs" role="tablist">
        ...
        >
          All
          <span class="ms-tab__count">{{ stats.total }}</span>
        </button>
        <button
            type="button"
            class="ms-tab"
        ...
        >
          Published
          <span class="ms-tab__count ms-tab__count--pub">{{ stats.published }}</span>
        </button>
        <button
            type="button"
            class="ms-tab"
        ...
        >
          Drafts
          <span class="ms-tab__count ms-tab__count--draft">{{ stats.draft }}</span>
        </button>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div v-else-if="scenarios.length" class="ms-noresults">
        ...
        <p class="ms-noresults__text">
          No scenarios match
          <strong>"{{ search || statusFilter }}"</strong>
        </p>
        <button type="button" class="ms-noresults__reset" @click="search = ''; statusFilter = 'ALL'">
          Clear filters
        </button>
      </div>
      ...
      <div v-else class="ms-empty">
        ...
        <p class="ms-empty__sub">
          A scenario is a storyboard of scenes — add images, record voices,<br>
          annotate with linguistic glosses and publish to the community.
        </p>
        <RouterLink to="/create-scenario" class="ms-empty__cta">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
               stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 5v14M5 12h14"/>
          </svg>
...
.ms-search:focus-within {
  ...
  box-shadow: 0 0 0 3px rgba(91, 25, 40, 0.1);
}
...
.ms-tab.active {
  background: #fff;
  color: var(--text);
  box-shadow: 0 1px 4px rgba(30, 8, 18, 0.1);
}
...
.ms-card:hover {
  ...
  box-shadow: 0 16px 40px rgba(30, 8, 18, 0.12);
...
.ms-card__placeholder-num {
  font-size: 2.5rem;
  font-weight: 950;
  color: rgba(30,8,18, 0.18);
...
.ms-card__badge--draft {
  background: rgba(255, 255, 255, 0.92);
  color: #8B3010;
  border: 1px solid rgba(72,91,56,0.3);
}
...
.ms-empty__cta svg { width: 16px; height: 16px; }
...
.ms-card__action--edit { padding: 6px 10px; }
.ms-card__action--edit svg { width: 14px; height: 14px; }
.ms-card__action--edit:hover { border-color: var(--primary); color: var(--primary); background: rgba(15,118,110,0.06); }
...
.ms-edit__input, .ms-edit__textarea {
  border: 1.5px solid #D4E5CA;
  border-radius: 10px;
  padding: 11px 13px;
  background: #fff;
